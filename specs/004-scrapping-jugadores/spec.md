...
- Q: Cuando una fuente devuelve una mezcla de registros válidos e incompletos, ¿qué debe hacer la actualización? → A: Conservar solo los registros válidos, omitir los incompletos y continuar actualizando el catálogo con la información confiable.
- Q: ¿La actualización debe requerir WhoScored y Football-Data.org, o puede utilizar una sola fuente? → A: En esta entrega se utilizará únicamente WhoScored; Football-Data.org queda fuera del alcance.
...
- Si una fuente externa devuelve datos incompletos, la actualización debe conservar los registros válidos, omitir los incompletos y no vaciar el conjunto existente.
...
- **FR-001**: El sistema MUST permitir actualizar las estadísticas de los jugadores a partir de WhoScored sin requerir que la base ya esté poblada.
- **FR-002**: El sistema MUST extraer y persistir para cada jugador al menos nombre, equipo, liga, edad, goles, asistencias, disparos por partido, porcentaje de pases exitosos, dribbles exitosos, faltas cometidas y rating.
- **FR-003**: La información actualizada MUST almacenarse en la entidad existente Jugador como fuente local de consulta del catálogo. Se permite registrar metadatos técnicos de ejecución en una entidad auxiliar de auditoría, siempre que no reemplace ni fragmente la información del jugador.
...
- **FR-005**: Si WhoScored falla o devuelve datos inválidos, el sistema MUST seguir funcionando con la última información local válida y MUST evitar destruir datos ya almacenados.
- **FR-005a**: Si una respuesta contiene registros válidos e incompletos, el sistema MUST persistir los registros válidos, omitir los incompletos y continuar el proceso sin reemplazar información local válida por datos incompletos.
...
- **FR-011**: El sistema MUST incluir la información actual relevante del jugador en la base de datos para que la consulta posterior sea independiente de WhoScored.
...
- **Jugador**: Persona deportiva cuya información estadística se actualiza y persiste; incluye nombre, equipo, liga, edad y métricas relevantes como goles, asistencias, disparos por partido, porcentaje de pases exitosos, dribbles exitosos, faltas cometidas y rating.
- **WhoScored**: Fuente externa única de estadísticas para esta entrega; entrega información del rendimiento del jugador y debe tratarse como una dependencia no confiable del sistema.
...
- **Snapshot de actualización**: Registro técnico opcional del resultado de cada intento de actualización; sirve para diagnóstico y auditoría, pero no forma parte del catálogo consultable de jugadores.
...
- **SC-002**: El 100% de los jugadores actualizados incluye los campos obligatorios de nombre, equipo, liga, edad, goles, asistencias, disparos por partido, porcentaje de pases exitosos, dribbles exitosos, faltas cometidas y rating.
...
- La actualización de estadísticas depende exclusivamente de WhoScored en esta entrega; la fuente puede fallar o devolver datos incompletos, por lo que la última versión local válida debe mantenerse como respaldo.
